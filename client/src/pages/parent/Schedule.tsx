...
import { type Schedule, type ScheduleSubstitution, type RescheduleProposalWithRelations } from "@shared/schema";
import { z } from "zod";
import { ChevronLeft, ChevronRight, ChevronUp, ChevronDown, Calendar, Clock, MapPin, CalendarDays, Bell, FileText, User, UserCheck, RefreshCw, Link as LinkIcon } from "lucide-react";
import { RescheduleProposalModal, PendingProposals } from "@/components/RescheduleProposalModal";
import { GoogleCalendarButton } from "@/components/GoogleCalendarButton";
import { format, startOfWeek, addDays, isSameDay, addWeeks, subWeeks, parseISO, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, addMonths, subMonths, endOfWeek, subDays, startOfDay, getISOWeek } from "date-fns";
...
export default function ParentSchedule({ parentId }: ParentScheduleProps) {
  ...
  const getActualStatus = (schedule: Schedule): string => {
    ...
    // If it's rescheduled but hasn't started yet, it's rescheduled
  ...
  const cancelMutation = useMutation({
    mutationFn: async (id: string) => {
      return await apiRequest("PATCH", `/api/schedules/${id}`, { status: "cancelled", cancelledBy: parentId });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/parents", parentId, "schedules"] });
  ...
  const getSchedulesForDay = (day: Date) => {
    return schedules.filter(schedule => 
      isSameDay(parseISO(schedule.startTime as string), day)
    );
  };
  ...
  const getStudentNameForSchedule = (schedule: ScheduleWithSubstitution) => {
    // First, use the studentName from the API if available
    if (schedule.studentName) {
      return schedule.studentName;
    }
    
    // Fallback: Find which child is enrolled in this course
    const enrollment = enrollments.find(e => e.courseId === schedule.courseId && e.status === 'enrolled');
    if (enrollment) {
    ...
    for (const child of children) {
      const childEnrollment = enrollments.find(e => 
        e.courseId === schedule.courseId && e.studentId === child.childId && e.status === 'enrolled'
      );
      if (childEnrollment) {
        const student = users.find(u => u.id === child.childId);
        if (student) {
          return `${student.firstName || ''} ${student.lastName || ''}`.trim() || student.name || 'Unknown';
        }
      }
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto" data-testid="parent-schedule-page">
      {/* Pending Reschedule Proposals */}
      <PendingProposals userId={parentId} invalidateQueries={invalidateScheduleQueries} proposals={pendingProposals} />

      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2" data-testid="text-schedule-title">Family Schedule</h1>
          <p className="text-muted-foreground" data-testid="text-schedule-subtitle">
            View and reschedule courses for your children
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {isLoading ? (
                ...
                upcomingSchedules.map((schedule) => {
                  ...
                  const studentName = getStudentNameForSchedule(schedule);
            ...
            {daySchedules.length === 0 ? (
              <div className="py-20 flex flex-col items-center justify-center text-center px-4 bg-gradient-to-b from-card/50 to-transparent rounded-3xl border border-dashed border-border/60">
                ...
                <p className="text-sm text-muted-foreground max-w-md">
                  No classes scheduled for your children on {format(currentDate, 'MMMM d, yyyy')}
                </p>
              </div>
            ) : (
                                  ...
                                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
                                    ...
                                    {hasSubstitute(schedule) ? (
                                      <div className="flex items-center gap-1 text-amber-600 dark:text-amber-400 font-medium">
                                        ...
                                        <span>Sub: {getSubstituteTeacherName(schedule.substitution!.substituteTeacherId)}</span>
                                      </div>
                                    ) : (
                                      <div className="flex items-center gap-1">
                                        <User className="w-3.5 h-3.5" />
      ...
      {view !== 'month' && upcomingSchedules.length > 0 && (
          ...
          <CardContent>
            <div className="space-y-2">
              {upcomingSchedules.map((schedule) => {
                ...
                <div 
                  key={schedule.id}
                  className="flex items-center justify-between p-3 rounded-lg bg-background border hover-elevate cursor-pointer transition-all"
                  onClick={() => handleViewDetails(schedule)}
                  data-testid={`upcoming-${schedule.id}`}
                >
                          <div className="flex-1 min-w-0">
                            <h4 className="font-semibold text-sm truncate">{courseInfo.title}</h4>
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-xs text-muted-foreground">
                              ...
                              {hasSubstitute(schedule) ? (
                                <span className="flex items-center gap-1 text-amber-600 dark:text-amber-400">
                                  ...
                                  Sub: {getSubstituteTeacherName(schedule.substitution!.substituteTeacherId)}
                                </span>
                              ) : (
                                <span className="flex items-center gap-1">
                                  <User className="w-3 h-3" />
                              ...
                              {schedule.externalLink && new Date(schedule.endTime) > new Date() && schedule.status !== 'completed' && schedule.status !== 'cancelled' && (
                                <Button
                                  ...
                                  size="sm"
                                  asChild
                                  className="h-6 px-2 text-xs font-semibold bg-gradient-to-r from-primary to-primary/80 hover:shadow-lg hover:shadow-primary/50 transition-all"
                          ...
                          <div className="flex flex-col items-end gap-1 flex-shrink-0">
                            ...
                            {schedule.status === "cancelled" && schedule.cancelledBy && (() => {
                  ...
                  {daySchedules.length === 0 ? (
                    ...
                    daySchedules
                      ...
                      .map((schedule) => (
                        <div 
                          key={schedule.id}
                          className="p-3 rounded-lg bg-muted/30 cursor-pointer transition-all hover-elevate overflow-hidden"
                          onClick={() => handleViewDetails(schedule)}
                        ...
                        >
                          <div className="flex items-start justify-between gap-2 mb-2 min-w-0">
                            ...
                            <div className="flex flex-col items-end gap-1 flex-shrink-0 max-w-[85px] overflow-hidden">
                              <div className="flex gap-1 justify-end flex-wrap overflow-hidden">
            ...
            <>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex flex-col items-end gap-1 flex-shrink-0">
                      ...
                      {getActualStatus(selectedSchedule) === "cancelled" && selectedSchedule.cancelledBy && (() => {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-semibold" data-testid="text-student-name">
                        {getStudentNameForSchedule(selectedSchedule)}
                      </p>
                    </div>
                  </div>
                </div>
                      ...
                      <div className="flex items-center gap-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700">
                        ...
                        <div className="flex-1">
                          ...
                          {selectedSchedule.substitution!.reason && (
                            <p className="text-xs text-amber-600/80 dark:text-amber-400/80 mt-1" title={selectedSchedule.substitution!.reason}>
              ...
              <div className="flex justify-between pt-4 border-t">
                ...
                {new Date(selectedSchedule.endTime) > new Date() && 
                 getActualStatus(selectedSchedule) !== "cancelled" && 
                 getActualStatus(selectedSchedule) !== "completed" && (
