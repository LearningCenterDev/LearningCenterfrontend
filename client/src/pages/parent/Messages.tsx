...
export default function ParentMessages({ parentId }: ParentMessagesProps) {
  const { data: currentUser } = useQuery<User>({
    queryKey: ["/api/users", parentId],
    enabled: !!parentId,
  });

  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800" data-testid="parent-messages-page">
      {/* Premium Header */}
      <div className="bg-gradient-to-r from-[#1F3A5F] via-[#1F3A5F] to-[#2a4a75] relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <p className="text-white/60 text-sm sm:text-base">Communicate with teachers and stay informed about your child's progress</p>
        </div>
      </div>
        ...
        <div className="relative">
          ...
          <div className="relative bg-gradient-to-br from-white via-white to-slate-50/50 dark:from-slate-800/95 dark:via-slate-800 dark:to-slate-900/95 rounded-xl shadow-lg border border-slate-200/50 dark:border-slate-700/50 overflow-hidden backdrop-blur-sm">
            ...
            <div className="p-3 sm:p-4 md:p-5">
              <MessagingInterface currentUserId={parentId} />
            </div>
          </div>
        </div>
      </div>
